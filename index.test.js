...
const { calculateEmissions, handleFormSubmit, buildInsights } = require("./app.js");

describe("calculateEmissions - Symmetrical Guard Testing", () => {
  // Symmetrical type guards: check distance when coefficient is valid (0.24)
  describe("Invalid distance arguments (with valid coefficient)", () => {
    test("string distance returns null", () => {
      expect(calculateEmissions("10", 0.24)).toBeNull();
    });
    test("empty string distance returns null", () => {
      expect(calculateEmissions("", 0.24)).toBeNull();
    });
    test("alphabetic string distance returns null", () => {
      expect(calculateEmissions("abc", 0.24)).toBeNull();
    });
    test("null distance returns null", () => {
      expect(calculateEmissions(null, 0.24)).toBeNull();
    });
    test("undefined distance returns null", () => {
      expect(calculateEmissions(undefined, 0.24)).toBeNull();
    });
    test("boolean true distance returns null", () => {
      expect(calculateEmissions(true, 0.24)).toBeNull();
    });
    test("boolean false distance returns null", () => {
      expect(calculateEmissions(false, 0.24)).toBeNull();
    });
    test("object distance returns null", () => {
      expect(calculateEmissions({}, 0.24)).toBeNull();
    });
    test("array distance returns null", () => {
      expect(calculateEmissions([], 0.24)).toBeNull();
    });
    test("function distance returns null", () => {
      expect(calculateEmissions(() => {}, 0.24)).toBeNull();
    });
  ...
  // Symmetrical type guards: check coefficient when distance is valid (10)
  describe("Invalid coefficient arguments (with valid distance)", () => {
    test("string coefficient returns null", () => {
      expect(calculateEmissions(10, "0.24")).toBeNull();
    });
    test("empty string coefficient returns null", () => {
      expect(calculateEmissions(10, "")).toBeNull();
    });
    test("alphabetic string coefficient returns null", () => {
      expect(calculateEmissions(10, "abc")).toBeNull();
    });
    test("null coefficient returns null", () => {
      expect(calculateEmissions(10, null)).toBeNull();
    });
    test("undefined coefficient returns null", () => {
      expect(calculateEmissions(10, undefined)).toBeNull();
    });
    test("boolean true coefficient returns null", () => {
      expect(calculateEmissions(10, true)).toBeNull();
    });
    test("boolean false coefficient returns null", () => {
      expect(calculateEmissions(10, false)).toBeNull();
    });
    test("object coefficient returns null", () => {
      expect(calculateEmissions(10, {})).toBeNull();
    });
    test("array coefficient returns null", () => {
      expect(calculateEmissions(10, [])).toBeNull();
    });
    test("function coefficient returns null", () => {
      expect(calculateEmissions(10, () => {})).toBeNull();
    });
    test("NaN coefficient returns null", () => {
      expect(calculateEmissions(10, NaN)).toBeNull();
    });
    test("Infinity coefficient returns null", () => {
      expect(calculateEmissions(10, Infinity)).toBeNull();
    });
    test("-Infinity coefficient returns null", () => {
      expect(calculateEmissions(10, -Infinity)).toBeNull();
    });
    test("negative coefficient returns null", () => {
  ...
  describe("Both arguments invalid", () => {
    ...
    test("both strings returns null", () => {
      expect(calculateEmissions("10", "0.24")).toBeNull();
    });
  });
});
...
describe("calculateEmissions - Decimal Boundary and Valid Output Testing", () => {
  test("zero distance and valid coefficient returns 0", () => {
    expect(calculateEmissions(0, 0.24)).toBe(0);
  });

  test("valid distance and zero coefficient returns 0", () => {
    expect(calculateEmissions(10, 0)).toBe(0);
  });

  test("car emission calculation: 10 km × 0.24 = 2.4", () => {
    expect(calculateEmissions(10, 0.24)).toBe(2.4);
  });

  test("bus emission calculation: 10 km × 0.08 = 0.8", () => {
    expect(calculateEmissions(10, 0.08)).toBe(0.8);
  });

  test("decimal precision: 3.333 km × 0.24 = 0.80", () => {
    expect(calculateEmissions(3.333, 0.24)).toBe(0.8);
  });

  test("decimal precision: 7.5 km × 0.08 = 0.60", () => {
    expect(calculateEmissions(7.5, 0.08)).toBe(0.6);
  });

  test("IEEE 754 precision boundary: 1.005 km × 1.00 = 1.00", () => {
    expect(calculateEmissions(1.005, 1.00)).toBe(1.00);
  });

  test("fractional distance rounding: 1.23456 km × 0.24 = 0.30", () => {
    expect(calculateEmissions(1.23456, 0.24)).toBe(0.3);
  });

  test("high precision coefficients: 15 km × 0.123456 = 1.85", () => {
    expect(calculateEmissions(15, 0.123456)).toBe(1.85);
  });
});
...
describe("DOM Controller Architecture", () => {
  ...
  test("Submitting when pure calculation engine fails with null returns distance error", () => {
    ...
    // We bypass validation by setting distanceInput.value to a float which validates successfully,
    // but we inject a string "10" dynamically as distance to trigger calculateEmissions returning null
    elements.distanceInput.value = "10";
    
    // Temporarily replace dataset coeff with string to bypass validation (isNaN checks) but fail in calculateEmissions
